...
public interface FlashcardRepository extends JpaRepository<Flashcard, Long> {
    ...
    List<Flashcard> findByPublicCardTrueAndCategoryIn(List<String> categories);


    @Query("SELECT DISTINCT f.category FROM Flashcard f WHERE f.owner = :user OR f.publicCard = true")
    ...
    @Query("SELECT f FROM Flashcard f WHERE f.publicCard = true AND f.category IN :categories")
    List<Flashcard> findPublicByCategories(@Param("categories") List<String> categories);


    @Query("SELECT f FROM Flashcard f WHERE f.category IN :categories AND (f.owner = :user OR f.publicCard = true)")
    ...
    @Query("SELECT DISTINCT f.category FROM Flashcard f WHERE f.owner = :user OR f.publicCard = true")
    List<String> findDistinctCategoriesByOwnerOrPublic(@Param("user") User user);
}
